    ...
    try {
      ...
      if (strategy === 'minimal') {
        ...
        if (improved && typeof improved === 'string') {
          const normalized = String(improved).replace(/\r\n/g, '\n');
...
async function buildPreviewForSingleFile(patchRequestId, filePath) {
  ...
  patch.preview.files[stubIndex].originalText = originalLines.join('\n');
  patch.preview.files[stubIndex].improvedText = improvedLines.join('\n');
